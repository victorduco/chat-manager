...
DAILY_RUNNER_VOICE_PROBABILITY="${DAILY_RUNNER_VOICE_PROBABILITY:-0.25}"
SKIP_WEEKEND_RULE="${SKIP_WEEKEND_RULE:-0}"

{
  echo
  echo "===== $(date -u +"%Y-%m-%dT%H:%M:%SZ") daily_runner start ====="
  echo "ONLY_ENABLED=$ONLY_ENABLED FORCE=$FORCE LIMIT=$LIMIT BOOTSTRAP_ENABLE_N=$BOOTSTRAP_ENABLE_N ASSISTANT_ID=$ASSISTANT_ID LANGGRAPH_API_URL=$LANGGRAPH_API_URL DAILY_RUNNER_VOICE_PROBABILITY=$DAILY_RUNNER_VOICE_PROBABILITY SKIP_WEEKEND_RULE=$SKIP_WEEKEND_RULE"
} | tee -a "$LOG_FILE"
...
export DAILY_RUNNER_VOICE_PROBABILITY
set +a
...
async def main():
    ...
    result = await run_daily(
        ...
        skip_weekend_rule=_b("SKIP_WEEKEND_RULE", False),
    )
    print("RESULT:", result)
